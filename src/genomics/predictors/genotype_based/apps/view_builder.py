...
def _yaml_block(value: Any, indent: int = 2) -> str:
    space = " " * indent
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if isinstance(item, (dict, list)):
                ...
                lines.append(_yaml_block(item, indent + 2))
            else:
...
def _dataset_input_yaml_snippet(view: Dict[str, Any]) -> str:
    dataset_input = {
        key: value
        for key, value in view.items()
        if key not in {"name", "description"}
    }
    return "dataset_input:\n" + _yaml_block(dataset_input, indent=2)
...
class ViewBuilderHandler(BaseHTTPRequestHandler):
    ...
    def _handle_save(self, payload: Dict[str, Any]) -> None:
        ...
        yaml_snippet = _dataset_input_yaml_snippet(preview["view"])
        _json_response(
            self,
            {
                "saved": True,
