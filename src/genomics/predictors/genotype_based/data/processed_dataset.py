...
from genomics.predictors.genotype_based.utils import taint_sample
from genomics.core.targets import (
    build_class_maps,
...
class ProcessedGenomicDataset(Dataset):
    ...
    def _process_window_haplotype_channels(
        ...
        expanded_slice = self._expanded_slice_for_gene(window_name)
        signal_rows = []
        shared_masks = None
        if array.ndim == 2:
            if track_meta:
    ...
    def _global_variation_mask_for_gene(self, gene_name: str, expanded_slice: Tuple[int, int]) -> np.ndarray:
        ...
        if self.bcftools_chain_mapper is None:
            raise ValueError("Mascara global de variacao requer alignment_mapping='bcftools_chain'")
    ...
    def _expanded_slice_for_gene(self, gene_name: str) -> Tuple[int, int]:
        cached = self._expanded_slice_cache.get(gene_name)
        if cached is not None:
            return cached
        center_slice = self.dynamic_indel_aligner.get_reference_centered_expanded_slice(gene_name, self.window_center_size)
        cached = (int(center_slice["expanded_start"]), int(center_slice["expanded_end"]))
        self._expanded_slice_cache[gene_name] = cached
        return cached
    ...
    def _process_windows_haplotype_channels(self, windows: Dict, sample_id: Optional[str] = None) -> np.ndarray:
        ...
        for gene_name in gene_names:
            ...
            if self.alphagenome_signal_variant_mask and self.feature_mode != "masks_only":
                expanded_slice = self._expanded_slice_for_gene(gene_name)
    ...
    def _normalize_features_tensor(self, features_tensor: torch.Tensor) -> torch.Tensor:
        ...
        if per_track:
            ...
            if row_mask.any():
                ...
                features_tensor = normalized.view_as(features_tensor)
            self.profile_stats["normalization_s"] += time.perf_counter() - t0
            return features_tensor
        ...
        for channel_idx in range(features_tensor.shape[1]):
            gene_channel_idx = channel_idx % channels_per_gene if channels_per_gene > 0 else channel_idx
            channel_mask.append(gene_channel_idx < signal_channels_per_gene)
        if any(channel_mask):
            normalized[:, channel_mask, :] = apply_normalization(features_tensor[:, channel_mask, :], self.normalization_params)
    ...
    def get_input_shape(self) -> Tuple[int, int]:
        ...
        if self.config.dataset_input.tensor_layout == "haplotype_channels":
            ...
            num_genes = len(self.config.dataset_input.genes_to_use or []) or 1
            num_haplotypes = 2 if self.haplotype_mode == "H1+H2" else 1
            return (num_haplotypes * self._rows_per_gene() * num_genes, effective)
...
class CachedProcessedDataset(Dataset):
    ...
    def __init__(
        ...
        self.cache_size = 0 if self._shard_index is not None else config.data_loading.cache_size
        self.shard_cache_size = int(getattr(config.data_loading, "shard_cache_size", 4) or 0)
        self._input_shape_hint: Optional[Tuple[int, int]] = self._load_input_shape_hint()
        self.profile_stats = {
            "getitem_calls": 0,
            "item_cache_hits": 0,
            "item_cache_misses": 0,
        ...
        if self.loading_strategy == "preload" and self._shard_index is None:
            ...
            self._shard_cache: OrderedDict[Path, List[Tuple[torch.Tensor, torch.Tensor]]] = OrderedDict()
    ...
    def _load_input_shape_hint(self) -> Optional[Tuple[int, int]]:
        meta_path = self.data_file.parent / "metadata.json"
        ...
        try:
            with open(meta_path) as f:
                meta = json.load(f)
            shape = meta.get("model_input_shape") or meta.get("feature_shape")
            if isinstance(shape, list) and len(shape) == 2:
                hint = (int(shape[0]), int(shape[1]))
                expected = self._expected_model_input_shape()
    ...
    def _expected_model_input_shape(self) -> Optional[Tuple[int, int]]:
        di = self.config.dataset_input
        effective = int(di.window_center_size) // max(int(di.downsample_factor), 1)
        num_genes = len(di.genes_to_use or []) or 1
        if di.tensor_layout == "haplotype_channels":
            mask_channels = 2 + int(di.indel_include_valid_mask) + int(di.indel_include_snp_mask)
            if di.feature_mode == "masks_only":
                rows_per_gene_per_hap = mask_channels
            elif di.feature_mode == "signals_only":
                rows_per_gene_per_hap = len(di.ontology_terms or []) or 1
            else:
                rows_per_gene_per_hap = (len(di.ontology_terms or []) or 1) + mask_channels
            num_haplotypes = 2 if di.haplotype_mode == "H1+H2" else 1
            return num_haplotypes * rows_per_gene_per_hap * num_genes, effective
        if di.tensor_layout == "raw_center_crop":
            return None
        return None
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        if self.loading_strategy == "preload":
            ...
            if idx in self._cache:
                self.profile_stats["item_cache_hits"] += 1
                features, target = self._cache[idx]
            else:
                self.profile_stats["item_cache_misses"] += 1
                if self._shard_index is None:
                    if not self._data_loaded or self.data is None:
                        t0 = time.perf_counter()
                        self.data = torch.load(self.data_file)
                        self.profile_stats["shard_load_s"] += time.perf_counter() - t0
                        self.profile_stats["shard_load_calls"] += 1
                        self._data_loaded = True
                    features, target = self.data[idx]
                else:
                    features, target = self._get_item_from_shards(idx)
    ...
    def get_input_shape(self) -> Tuple[int, int]:
        if self._input_shape_hint is not None:
            return self._input_shape_hint
    ...
    def _get_item_from_shards(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._loaded_shard_path != shard_path or self.data is None:
            cached_shard = self._shard_cache.get(shard_path)
            if cached_shard is not None:
                self.profile_stats["shard_cache_hits"] += 1
                self._shard_cache.move_to_end(shard_path)
                self.data = cached_shard
            else:
                self.profile_stats["shard_cache_misses"] += 1
                t0 = time.perf_counter()
                self.data = torch.load(shard_path)
                self.profile_stats["shard_load_s"] += time.perf_counter() - t0
                self.profile_stats["shard_load_calls"] += 1
                if self.shard_cache_size > 0:
                    self._shard_cache[shard_path] = self.data
                    self._shard_cache.move_to_end(shard_path)
