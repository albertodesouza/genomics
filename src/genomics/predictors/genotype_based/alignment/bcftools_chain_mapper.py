...
def run_bcftools_consensus_with_chain(
    *,
    ...
    if shutil.which("bcftools") is None:
        raise RuntimeError("bcftools nao encontrado no PATH. Rode source scripts/env/start_genomics_universal.sh")
...
class BcftoolsChainMapper:
    def __init__(
        ...
        *,
        ...
        self._axis_cache: Dict[str, Dict[str, object]] = {}
        self._axis_key_cache: Dict[str, str] = {}
        self._sample_set_key_cache: Dict[str, str] = {}
        self._ref_seq_cache: Dict[str, str] = {}
        self.profile_stats = {
            "entry_memory_hits": 0,
            "entry_disk_hits": 0,
            "entry_misses": 0,
    ...
    def _axis(self, gene: str) -> Dict[str, object]:
        cached = self._axis_cache.get(gene)
        if cached is None:
            cached = self.aligner.get_alignment_axis(gene)
            self._axis_cache[gene] = cached
        return cached
    ...
    def _ref_path(self, gene: str) -> Path:
        return self.consensus_dataset_dir / "references" / "windows" / gene / "ref.window.fa"

    def _axis_cache_key(self, gene: str) -> str:
        cached = self._axis_key_cache.get(gene)
        if cached is not None:
            return cached
        axis = self._axis(gene)
        cached = (
        ...
        self._axis_key_cache[gene] = cached
        return cached

    def _sample_set_key(self, gene: str) -> str:
        cached = self._sample_set_key_cache.get(gene)
        if cached is not None:
            return cached
        axis = self._axis(gene)
        cached = str(axis.get("sample_set_key", "samples_unspecified"))
        self._sample_set_key_cache[gene] = cached
        return cached

    def _gene_cache_dir(self, gene: str) -> Path:
        return self.cache_dir / self._sample_set_key(gene) / "genes" / gene

    def _entry_cache_path(self, gene: str, sample_id: str, haplotype: str, axis_key: str) -> Path:
        return self._gene_cache_dir(gene) / "entries" / axis_key / sample_id / f"{haplotype}.entry.json"
    ...
    def get_haplotype_entry(self, gene: str, sample_id: str, haplotype: str) -> Optional[Dict[str, object]]:
        ...
        if key in self._entry_cache:
            self.profile_stats["entry_memory_hits"] += 1
            self._entry_cache.move_to_end(key)
        ...
        if path.exists():
            try:
                ...
                with open(path) as f:
                    payload = json.load(f)
                self.profile_stats["entry_disk_read_s"] += time.perf_counter() - t0
                if payload.get("version") == BCFTOOLS_CHAIN_MAPPER_VERSION:
                    entry = payload.get("entry")
                    if isinstance(entry, dict):
                        self.profile_stats["entry_disk_hits"] += 1
                        self._remember_entry(key, entry)
                        return entry
            except Exception:
                pass
        self.profile_stats["entry_misses"] += 1
        ...
        ref_seq = self._ref_seq_cache.get(gene)
        if ref_seq is None:
            ref_seq = read_fasta_sequence(self._ref_path(gene))
            self._ref_seq_cache[gene] = ref_seq
        ...
        with open(path, "w") as f:
            json.dump({"version": BCFTOOLS_CHAIN_MAPPER_VERSION, "entry": entry}, f, separators=(",", ":"))
        self.profile_stats["entry_build_s"] += time.perf_counter() - build_t0
        self.profile_stats["entry_builds"] += 1
        self._remember_entry(key, entry)
        return entry

    def _build_validated_consensus(self, case_dir: Path, sample_id: str, haplotype: str) -> Tuple[Path, Path]:
        gene = case_dir.name
        cache_dir = self._gene_cache_dir(gene) / "bcftools" / sample_id / haplotype
        raw_path = case_dir / f"{sample_id}.{haplotype}.window.raw.fa"
        fixed_path = case_dir / f"{sample_id}.{haplotype}.window.fixed.fa"
        ref_path = self._ref_path(gene)
...
def main() -> None:
    ...
    raw_rebuilt, chain_path = run_bcftools_consensus_with_chain(
        case_dir=case_dir,
        ref_path=args.consensus_root / "references" / "windows" / args.gene / "ref.window.fa",
    ...
    ref_seq = read_fasta_sequence(args.consensus_root / "references" / "windows" / args.gene / "ref.window.fa")
