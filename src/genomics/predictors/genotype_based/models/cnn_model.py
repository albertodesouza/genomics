...
from .nn_model import _resolve_gene_rows, _make_activation
...
class CNNAncestryPredictor(nn.Module):
    ...
    def __init__(self, config: PipelineConfig, input_shape: Tuple[int, int], num_classes: int):
        ...
        self.use_row_selection = self.rows_to_use != list(range(input_shape[0]))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim == 4:
            ...
            raise ValueError(f"CNNAncestryPredictor espera entrada (B,rows,L) ou (B,2,rows,L), recebeu shape={tuple(x.shape)}")
        if self.use_row_selection:
