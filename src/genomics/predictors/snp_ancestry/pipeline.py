...
DEFAULT_ASSET_DIR = REPO_ROOT / "configs" / "predictors" / "snp_ancestry"
DEFAULT_RESULTS_DIR = REPO_ROOT / "results" / "snp_ancestry_predictor"
from genomics.converters.vcf_to_23andme.converter import (
    normalize_chrom,
    generate_header,
...
def load_config(path: Path) -> dict:
    ...
    _apply_dataset_registry_defaults(cfg)
    return cfg


def _apply_dataset_registry_defaults(config: dict) -> None:
    inp = config.setdefault("input", {})
    dataset_id = inp.get("dataset_id")
    if not dataset_id:
        return

    ref = resolve_dataset(str(dataset_id))
    dataset_dir = ref.path
    inp.setdefault("dataset_dir", str(dataset_dir))
    inp.setdefault("individuals_dir", str(dataset_dir / "individuals"))

    if not inp.get("vcf_pattern") and ref.metadata_path.exists():
        try:
            with open(ref.metadata_path) as f:
                metadata = json.load(f)
            raw_sources = metadata.get("raw_variant_sources", {}) or {}
            if raw_sources.get("vcf_pattern"):
                inp["vcf_pattern"] = raw_sources["vcf_pattern"]
        except Exception:
            pass
...
def require_bcftools(console: Console) -> None:
    ...
    try:
        ...
        console.print("  source scripts/env/start_genomics_universal.sh")
        sys.exit(1)
...
def _build_statistics_path(config: dict) -> str:
    ...
    output_dir = stat_cfg.get(
        "output_dir",
        stat_cfg.get("output_file", str(DEFAULT_RESULTS_DIR)),
    )
