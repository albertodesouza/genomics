...
def _load_alphagenome_modules():
    try:
        from alphagenome.data import gene_annotation
        from alphagenome.models import dna_client
    except ImportError as exc:
        raise ImportError(
            "AlphaGenome is required for gene lookup, tissue listing, and predictions. "
...
def _parse_csv_arg(value: Optional[str]) -> list:
    if not value:
        return []
...
def get_gene_interval(gtf: pd.DataFrame, gene_symbol: Optional[str], gene_id: Optional[str]):
    ...
    gene_annotation, _dna_client = _load_alphagenome_modules()
    if gene_symbol:
...
def process_window(
    ...
    if not metadata_path.exists():
        with open(metadata_path, "w") as f:
            json.dump(
                {
                    "type": getattr(args, "mode", "unknown"),
                    "chromosome": chrom,
                    "start": actual_start - 1,
                    "end": actual_end,
                    "window_size": actual_end - (actual_start - 1),
                    "requested_start": requested_start,
                    "requested_end": requested_end,
                    "requested_window_size": window_size,
                    "outputs": _parse_csv_arg(getattr(args, "outputs", None)),
                    "ontologies": _parse_csv_arg(getattr(args, "ontology", None)),
                    "raw_variant_source": {
                        "chromosome": chrom,
