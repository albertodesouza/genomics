...
class IndividualDatasetBuilder:
    ...
    def __init__(
        ...
        self.references_windows_dir = self.base_dir / "references" / "windows"
    ...
    def create_structure(self) -> None:
        ...
        self.references_windows_dir.mkdir(parents=True, exist_ok=True)
    ...
    def add_window(
        ...
        metadata_path = self.references_windows_dir / target_name / "window_metadata.json"
    ...
    def validate_window_files(self, target_name: str) -> Dict[str, bool]:
        ...
        validation = {
            ...
            'ref_fasta': (self.references_windows_dir / target_name / 'ref.window.fa').exists(),
            'window_metadata': (self.references_windows_dir / target_name / 'window_metadata.json').exists(),
...
class DatasetMetadataBuilder:
    ...
    def scan_individuals(self) -> None:
        ...
        ref_windows_dir = self.dataset_dir / "references" / "windows"
        if ref_windows_dir.exists():
            for metadata_path in sorted(ref_windows_dir.glob("*/window_metadata.json")):
                try:
                    with open(metadata_path, 'r') as f:
                        window_catalog[metadata_path.parent.name] = json.load(f)
                except Exception as e:
                    print(f"[WARN] Erro ao carregar {metadata_path}: {e}")
        ...
        for ind_dir in individual_dirs:
            ...
            try:
                ...
                # Coletar tipos de janela e outputs do catalogo global canonico.
                for window_name in windows:
                    window_info = window_catalog.get(window_name, {})
                    if not window_info:
                        continue
        ...
        self.metadata['window_catalog'] = {k: window_catalog[k] for k in sorted(window_catalog.keys()) if k in all_genes}
    ...
    def save_metadata(self) -> None:
        ...
        with open(self.metadata_file, 'w') as f:
            json.dump(self.metadata, f, indent=2)
        layout_metadata_file = self.dataset_dir / "layout_metadata.json"
        if not layout_metadata_file.exists():
            with open(layout_metadata_file, 'w') as f:
                json.dump(
                    {
                        "layout_version": 1,
