...
def load_frog_likelihoods(filepath: Path) -> pd.DataFrame:
    ...
    Example:
        >>> df = load_frog_likelihoods(Path("third_party/FROGAncestryCalc/output/whole_1000genomes_55aisnps_likelihood.txt"))
        >>> print(df.shape)
...
def load_population_mapping(filepath: Path) -> Dict[str, Dict[str, str]]:
    ...
    Example:
        >>> mapping = load_population_mapping(Path("third_party/FROGAncestryCalc/population_mapping_1000genomes.csv"))
