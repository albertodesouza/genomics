...
Self-contained (numpy + stdlib only) so it can be used from this notebook without
depending on the rest of the `genomics` package.
"""
...
def load_haplotype_variants(vcf_path: Path, haplotype: str) -> List[Tuple[int, str, str]]:
    ...
    `vcf_path` is expected to be a single-sample, biallelic-only VCF (the output of
    `consensus.build_haplotype_fasta`, the same VCF fed to `bcftools consensus`),
    ...
    with opener(vcf_path, "rt") as f:
        for line in f:
            ...
            if alleles[allele_index] != "1":
                continue
            if alt.startswith("<"):
                # Symbolic ALT (e.g. "<DEL>", left in by the consensus-ready filter):
                # bcftools consensus resolves its true extent from INFO/END, not from the
                # literal ALT string -- using len(alt) here (e.g. len("<DEL>") == 5) would
