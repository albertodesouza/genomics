...
# Prefer non-interactive backend before neural_ancestry_predictor_deprecated runs (DISPLAY can make it pick TkAgg).
...
from genomics_pipeline.sklearn_pca_cache import METADATA_FILENAME, ensure_sklearn_pca_cache, load_neural_ancestry_predictor_deprecated_for_cli
...
def _run_trials(
    ...
    for model in models:
        ...
        for params in _grid_product(grid):
            ...
            clf = build_sklearn_classifier(cfg_t, model, random_seed, n_train_samples=len(y_train))
            clf.fit(X_train, y_train)
            fit_s = time.perf_counter() - t0

            y_pred_tr = clf.predict(X_train)
            y_pred_va = clf.predict(X_val)
            m_tr = sklearn_metrics_dict(y_train, y_pred_tr, full_dataset)
            m_va = sklearn_metrics_dict(y_val, y_pred_va, full_dataset)
...
def main() -> None:
    ...
    nap = load_neural_ancestry_predictor_deprecated_for_cli()
    ...
    if not dataset_cache_dir.exists() or not validate_cache(dataset_cache_dir, config):
        nap.console.print(
            ...
            "[yellow]Gere o cache do dataset antes (treino neural_ancestry_predictor_deprecated).[/yellow]"
        )
        sys.exit(1)
