...
> *"Using AlphaGenome output processed by `neural_ancestry_predictor_deprecated.py` and `annotate_deeplift_windows.py`, it is possible to identify what in the DNA of Africans causes them to have greater skin pigmentation."*
...
1. **Train** `neural_ancestry_predictor_deprecated.py` with longevous vs non-longevous individuals
