...
This module is deprecated and kept only for historical reproducibility. The old active package name `neural_ancestry_predictor` has been retired; new experiments should use `genotype_based_predictor` through the shared CLI:
...
See `neural_ancestry_predictor_deprecated/DEPRECATED.md` for the migration map.

The migrated configs in `genotype_based_predictor/configs/neural_legacy/` use `tensor_layout: raw_center_crop` to reproduce the historical unaligned center-crop representation. New aligned genotype experiments should use `tensor_layout: haplotype_channels`.

Single-gene screen has also moved:

```bash
python3 -m genomics_cli genotype single-gene-screen genotype_based_predictor/configs/neural_legacy/pigmentation_binary_single_gene_screen.yaml
```
...
cd genomics/neural_ancestry_predictor_deprecated
...
```bash
python3 neural_ancestry_predictor_deprecated.py --config configs/genes_interp.yaml
```
...
```bash
python3 neural_ancestry_predictor_deprecated.py --config configs/pigmentation_binary.yaml
```
...
├── neural_ancestry_predictor_deprecated.py      # Main training/inference program
