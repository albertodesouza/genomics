#!/usr/bin/env python3
"""One classifier per gene, restricted to melanocyte of skin AND the gene's own strand.
...
CONFIG_DIR = REPO_ROOT / "configs/predictors/genotype_based/pigmentation/single_gene_mel_strand"
CACHE_PARENT = "results/cache/genotype_based_predictor/pigmentation_mel_strand"
CACHE_ROOT = REPO_ROOT / CACHE_PARENT / "datasets"
KO_DIR = REPO_ROOT / "results/genotype_based_predictor/knockout_bulk/melanocyte_strand"
...
EXPECTED_PARAMS = 49_522      # what a published three-ontology arm has
MIN_FREE_GB = 8
...
NOTE = """metadata:
  note: >
    SINGLE-GENE ARM, MELANOCYTE + GENE STRAND -- {GENE} alone, ontology CL:1000458
    ...
    scripts/experiments/single_gene_sweep_melanocyte_strand.py. Identical to
    pigmentation_binary_no_alignment.yaml in split, seed, crop, layout, optimisation and
...
def write_config(gene: str, strand: str) -> Path:
    ...
    di["results_dir"] = f"results/genotype_based_predictor/runs_single_gene_{gene.lower()}_melstrand"
    ...
    out = CONFIG_DIR / f"pigmentation_binary_single_{gene.lower()}_melstrand.yaml"
...
def verify_config(path: Path, gene: str, strand: str):
    ...
    if f"runs_single_gene_{gene.lower()}_melstrand" not in di["results_dir"]:
        raise SystemExit(f"ABORT: {path} has no isolated results_dir")
    if "pigmentation_mel_strand" not in di["processed_cache_dir"]:
...
def check_params(run_dir: Path, gene: str):
    """A wrong input-row count shows up here and nowhere else, so it is asserted."""
    import torch
    ...
    if n != EXPECTED_PARAMS:
        _log(f"{gene:<9} WARN: {n:,} parameters, expected {EXPECTED_PARAMS:,}. Stage-1 kernel "
             f"shapes {k[:1]}. A mismatch means the arm received the wrong number of input "
             f"rows -- treat this arm's numbers as invalid until explained.")
    return n
...
def main():
    ...
    for g in genes:
        arm = g.lower()
        cfg = CONFIG_DIR / f"pigmentation_binary_single_{arm}_melstrand.yaml"
        run_dir = REPO_ROOT / f"results/genotype_based_predictor/runs_single_gene_{arm}_melstrand" / RUN_NAME
        ...
        rc = run([PY, "-m", "genomics", "genotype", "train", str(cfg)],
                 LOG_DIR / f"melstr_{arm}_{stamp}.train.log")
        if rc != 0:
        ...
        rc = run([PY, "-m", "genomics", "genotype", "test", str(cfg)],
                 LOG_DIR / f"melstr_{arm}_{stamp}.test.log")
        if rc != 0:
        ...
        rc = run([PY, "scripts/experiments/single_gene_knockdown_replay_ontology.py",
                 ...
                 LOG_DIR / f"melstr_{arm}_{stamp}.replay.log")
        if rc != 0:
