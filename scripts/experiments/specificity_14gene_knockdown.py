#!/usr/bin/env python3
"""Rank pigmentation and NON-pigmentation genes side by side on one classifier.

The published knockdown ranking is over eleven genes that are all pigmentation genes, so it can
order them but cannot say what "unused" looks like for a gene that has no role in the phenotype
at all. `runs_specificity_14gene/` retrained the tracks-only classifier on the eleven-gene panel
plus three magnitude-matched, pigmentation-irrelevant windows (TPM2, SMCR8, PSMC4) chosen by
scripts/experiments/specificity_control_preflight.py. This runs the probe over all fourteen on
that checkpoint and ranks them.

WHY ALL FOURTEEN AND NOT JUST THE THREE. The retrain changes the classifier, so the eleven
published Delta values are not comparable to this run's. Like-for-like or nothing.

WHAT IS PAID FOR. The 100 bp biology_tss scrambles for the eleven panel genes are already in
notebooks/.cache/promoter_knockout_predictions (the cache key is
<sample>_<gene>_<hap>_biology_tss_<target_idx>_scramble100 and the target index is a
deterministic function of the reference TSS and the individual's own indel drift, so it is
identical to the published run's). Only the three control genes need new AlphaGenome calls:
162 individuals x 3 genes x 2 haplotypes = 972. The CAGE-anchored methods are NOT run here --
each would need its own per-haplotype CAGE prediction for the control windows, and biology_tss
is the paper's primary readout.

REPORTED PER GENE, ALWAYS TOGETHER (a flat gene means nothing without its delivered magnitude):
  |Delta|      mean absolute shift in log-odds(strong/weak) over the 162 test individuals
  |Delta_in|   delivered perturbation that actually reaches the CNN input, summed over
               haplotypes, measured through the real _process_window_haplotype_channels path
  ratio        |Delta| / |Delta_in| -- response per unit delivered

PRE-REGISTERED READING (fixed in the config before the retrain, unchanged here):
...
Usage:
  python3 scripts/experiments/specificity_14gene_knockdown.py
...
DEFAULT_OUT = REPO_ROOT / "results/genotype_based_predictor/knockout_bulk/pigmentation_test_split_knockout_14gene.csv"
METHOD = "biology_tss"
SCRAMBLE = 100
        ...
        for sid in sample_ids:
            for gene in genes:
                ...
                try:
                    ...
                    cached_all = True
                    for hap in ("H1", "H2"):
                        loc = haplotype_local_idx(dataset_dir, sid, gene, hap,
                        ...
                        key = f"{sid}_{gene}_{hap}_{METHOD}_{loc}_scramble{SCRAMBLE}"
